...
public class ServicioActividades {
    ...
    public List<ActividadAcademica> listarActividades() {
        ...
        for (Curso curso : servicioCursos.listarCursos()) {
            actividades.addAll(curso.getActividades());
        }
        actividades.sort(Comparator
                .comparing(ActividadAcademica::getFechaEntrega, Comparator.nullsLast(Comparator.naturalOrder()))
                .thenComparing(ActividadAcademica::getTitulo));
        return actividades;
    }

    public List<ActividadAcademica> filtrar(String cursoId, Prioridad prioridad) {
        List<ActividadAcademica> filtradas = new ArrayList<>();
        for (ActividadAcademica actividad : listarActividades()) {
            boolean coincideCurso = cursoId == null || cursoId.isBlank() || actividad.getCursoId().equals(cursoId);
            boolean coincidePrioridad = prioridad == null || actividad.getPrioridad() == prioridad;
            if (coincideCurso && coincidePrioridad) {
